...
"""1. A fast-food company charges £1.50 for delivery to your home, except for large orders of £10 or more where delivery 
is free. Write a function fast_food_order_price() that asks the user for the basic price of an order and prints a “The 
total price of your order is . . . ” message containing the total price including any delivery charges."""
def fast_food_order_price():
    price = eval(input("\nEnter the price of the food order: "))
    if price < 10:
        price += 1.50
...
numbers between these two values, shown to three decimal places. For example, the call display_square_roots(2, 4) should 
result in the following output: The square root of 2 is 1.414 The square root of 3 is 1.732 The square root of 4 is 
2.000."""
def display_square_roots(start=10, end=100):
    print()
    for i in range(start, end + 1):
        print("The square root of {} is {:.3f}".format(i, i ** 0.5))


"""4. A school teacher marks her pupils’ coursework out of 20, but needs to translate these marks to a grade of A, B, C,
or F (where marks of 16 or above get an A, marks between 12 and 15 result in a B, marks between 8 and 11 give a C, and 
marks below 8 get an F). Write a calculate_grade() function that takes a mark as a parameter and returns the 
corresponding grade as a single-letter string. If the parameter value is too big or too small, the function should 
return a mark of X."""
def calculate_grade(mark=18):
...
ticket for a 100 kilometre journey costs £18). However, senior citizens (people who are 60 or over) and children (15 or 
under) get a 40% discount. Write a ticket_price function that takes the journey distance and passenger age as parameters 
(both integers), and returns the price of the ticket in pounds (i.e. a float)."""
def ticket_price(distance=100, age=14):
...
"""7. Write a numbered_square function that has a parameter n and displays a “numbered” square of size n; e.g, a call 
...
point, radius and eye colour (a string) are given as parameters. Complete the draw_coloured_eye() function so that it 
draws an eye like those from last week, but for any given colour. Write another function eye_picker() that asks the user 
for the coordinates of the centre of an eye, its radius and colour. If the user inputs a valid eye colour (blue, grey, 
green or brown), then an appropriately coloured eye should be displayed in a graphics window. Otherwise, just a “not a 
valid eye colour” message should be output (Note: remember to avoid repetitive code)."""
...
"""9. This exercise and the following one help to prepare you for the Python assignment. The table below shows 10 
different “patch designs”. Each patch design features a regular arrangement of lines, circles, rectangles and/or text, 
and has dimensions of 100×100 pixels.

Write a function draw_patch_window() (without parameters) which displays, in a graphics window of size 100×100 pixels, 
the patch design which is labelled with the final digit of your student number. The patch design should be displayed in 
red, as in the table. It’s important that your program draws the patch design accurately, but don’t worry if one pixel 
is chopped off at the edge of the window. Note that you don’t need to draw a border around the patch design."""
...
which to draw the patch, the x and y coordinates of where the top-left corner of the patch should be, and the colour of 
the patch."""
...
"""Write another function draw_patchwork (without parameters) which uses draw_patch() to draw a blue “patchwork” three 
patches wide and two patches high in a graphics window of size 300 × 200 pixels."""
def draw_patchwork():
...
"""11. Write an eyes_all_around function that allows the user to plot exactly 30 eyes on a graphics window of dimensions 
500 by 500 by clicking on chosen centre points. All eyes should be of radius 30, but they should be of different 
colours: specifically, the colours should repeatedly cycle through the sequence “blue”, “grey”, “green” and “brown”. 
Your function should call draw_coloured_eye to draw each eye."""
def eyes_all_around():
...
supplied draw_circle function. Your function should then allow the user to click on the graphics window five times, 
representing the firing of five arrows – each click representing the point on the target that is aimed at.

Fluctuating atmospheric conditions should be considered – your solution should generate two random values representing 
the amount an arrow will move horizontally and vertically from the aimed position during its flight, and these values 
should be adjusted a little (again randomly) after each arrow.

Generating random numbers is easily accomplished using the randint function from the random module. This function takes 
two arguments and returns a random number between these two values. For example, try:

import random
for i in range(10):
    print(random.randint(1, 5))

Each time the user clicks, the function should (i) display a small black circle representing where the arrow hits, and 
(ii) record the number of points scored. The points awarded for each arrow (click) are as follows: 10 for the yellow 
area, 5 for red and 2 for blue. After the final arrow, the function should display the total score on the graphics 
window. (Hint: calculate distances by first importing your pract05.py file and using the distance_between_points 
function."""
def archery_game():
