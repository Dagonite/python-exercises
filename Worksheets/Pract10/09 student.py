...
-first name
-last name
-student ID - which is generated from the first letter of the forename and the surname (e.g. for Bob Smith the ID would 
be bsmith)
-email address - which follows the format studentID@university.ac.uk (e.g. Bob Smith would be bsmith@university.ac.uk)
-course
-fees payment status
-whether the student is active (i.e. if they are still considered a student)

The university administrators will need to be able to change the student’s course, record their fees as paid (the amount 
is not required to be stored), withdraw them as a student (i.e. make them inactive), re-activate the student, print all 
the information they hold on the student."""
class Student:
