...
def personal_greeting():
    ...
    name, outputs a personalised greeting. E.g. for user input 'Sam', the
    function should output the greeting 'Hello Sam, nice to see you!'.

    Note: spacing and punctuation.
    """
...
def formal_name():
    """
    2. Write a formal_name() function which asks the user to input their given
    name and family name, and then outputs a more formal version of their name.
    E.g. on input 'Sam' and 'Brown', the function should output 'S. Brown'.
...
def kilos_to_pounds():
    """
    3. Copy the kilos_to_pounds() conversion function from your pract01.py
    file. Modify this function so that its output takes the form of a message
    such as '12.34 kilos is equal to 27.15 pounds', where both the user's kilos
    value and the calculated pounds values are displayed to two decimal places.
    """
...
def generate_email():
    """
    4. Suppose the University decides that student email addresses should be
    ...
    email address given a student's details. If the user enters: Sam, Brown and
    2015. The function should output 'brow.s.15@myport.ac.uk'.
    """
    first_name = input("Enter your first name: ")
    last_name = input("Enter your last name: ")
...
def grade_test():
    """
    5. A teacher awards letter grades for test marks as follows: 8, 9, or 10
    marks give an A. 6 or 7 marks give B. 4 or 5 marks give C. 0, 1, 2, or 3
    marks all give F. Using string indexing, write a function, grade_test(),
    that asks the user for a mark (between 0 and 10) and displays the
    corresponding grade.
    """
...
def graphic_letters():
    """
    6. Write a function, graphic_letters(), that first asks the user to enter
    a word, opens a graphics window, and then allows the user to display the
...
def sing_a_song():
    ...
    For example, if the user enters the word 'dum' and the numbers 2 and 4, the
    function should then output the following song:
...
def exchange_table():
    """
    8. Write a function, exchange_table(), that gives a table of euros values
    and their equivalent values in pounds, using an exchange rate of 1.108 euros
    to the pound. The euros values should be 0, 1, 2, ..., 20, and should be
...
def make_acronym():
    ...
    and then displays an acronym (the first letters of the words in capitals)
    for that phrase. For example, if the user enters 'University of Portsmouth',
...
def rainfall_chart():
    """
    12. [harder] The file rainfall.txt from the unit web-site contains rainfall
    data in mm for several UK cities for a particular day, in the form:
    ...
    chart using one asterisk for each mm of rainfall; given the above data the
    output should be:
...
def rainfall_in_inches():
    """
    14. [harder] Write a rainfall_in_inches() function that reads the rainfall
    data from 'rainfall.txt', and outputs the data to a file named
    'rainfall_inches.txt' where all the mm values are converted to inches (there
    are 25.4mm in an inch). The inch values should be given to two decimal
    places, so the Portsmouth line will become:
...
def wc():
    """
    15. [harder] In Linux, there is a command called wc which reports the number
    of characters, words and lines in a file. Write a function wc() which
