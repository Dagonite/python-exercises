...
E.g. for user input Sam, the function should output the greeting Hello Sam, nice to see you! (Note: the details of the 
...
"""2. Write a formal_name() function which asks the user to input his/her given name and family name, and then outputs a 
more formal version of their name. E.g. on input Sam and Brown, the function should output S. Brown (again note the 
...
takes the form of a message such as “12.34 kilos is equal to 27.15 pounds”, where both the user’s kilos value and the 
calculated pounds values are displayed to two decimal places."""
def kilos_to_pounds():
...
surname, the first letter of their forename, and the final two digits of the year they entered the university, separated 
by dots. Write a function called generate_email() that outputs an email address given a student’s details. (E.g., if the 
user enters the following information: Sam, Brown and 2015, the function should output: brow.s.15@myport.ac.uk"""
def generate_email():
    first_name = input("\nEnter your first name: ")
    last_name = input("Enter your last name: ")
...
"""5. A teacher awards letter grades for test marks as follows: 8, 9 or 10 marks give an A, 6 or 7 marks give B, 4 or, 5 
marks give C, and 0, 1, 2 or 3 marks all give F. Using string indexing, write a function grade_test which asks the user 
for a mark (between 0 and 10) and displays the corresponding grade."""
def grade_test():
    mark = eval(input("\nEnter your mark between 0 and 10: "))
    grade = "FFFFCCBBAAA"
    print("Your grade is", grade[mark])


"""6. Write a function graphic_letters() which first asks the user to enter a word, opens a graphics window, and then 
...
song’s word, how many lines long the song should be, and how many times the word should be repeated on each line. For 
example, if the user enters the word “dum” and the numbers 2 and 4, the function should then output the following song 
...
"""8. Write a function exchange_table() that gives a table of euros values and their equivalent values in pounds, using 
an exchange rate of 1.108 euros to the pound. The euros values should be 0, 1, 2, . . . , 20, and should be right 
justified. The pounds values should be 6 right justified and given to two decimal places (i.e. with decimal points lined 
up and with pence values after the points)."""
def exchange_table():
    for euros in range(21):
        pounds = euros / 1.108
        print("\n{:2} {:6.2f}".format(euros, pounds))


"""9. Write a make_acronym() function that allows the user to enter a phrase, and then displays an acronym (the first 
letters of the words in capitals) for that phrase. For example, if the user enters “University of Portsmouth”, the 
function should display UOP (Hint: first use the split method to find the words in the inputted string)."""
def make_acronym():
    phrase = input("\nEnter the phrase: ").upper()
    words = phrase.split()
...
value by adding up the “values” of its letters (where ‘a’ is 1, ‘b’ is 2, . . . ‘z’ is 26). So, for example, “Sam” has 
the value 19 + 1 + 13 = 33."""
def name_to_number():
...
def file_in_caps():
    ...
    text_file = input("\nEnter the text file you want to use (try Battles - Atlas): ")
...
rainfall; e.g., given the above data the output should be:
   Portsmouth  *********
   London      *****
   Southampton ************
"""
...
"""13. Now write a graphical version graphical_rainfall_chart that displays a similar bar chart in a graphical window 
but uses filled rectangles instead of sequences of asterisks."""
...
data to a file rainfall_inches.txt where all the mm values are converted to inches (there are 25.4mm in an inch). The 
inch values should be given to two decimal places, so the Portsmouth line above will become: Portsmouth 0.35"""
